...
// Define a reactive property to store the active page
let activePage

// Check if the activePage is set to 'layouts' or 'colorScheme'
if (ref('layouts').value) {
  activePage = ref('layouts')
} else {
  activePage = ref('colorScheme')
}
...
<template>
    ...
    <aside class="flex h-screen w-20 flex-col items-center border-r border-gray-200 ">
      ...
      <nav class="flex flex-1 flex-col gap-y-4 pt-10">
        ...
        <a href="/recipeManagement" :class="{ 'active-icon': activePage === 'colorScheme' }" @click="setActivePage('colorScheme')" class="group relative rounded-xl p-2">
      ...
      <div class="flex flex-col items-center gap-y-4 py-10">
        ...
        <button class="mt-2 rounded-full bg-gray-100">
          <img class="h-10 w-10 rounded-full" src="../../assets/img/profile.png" alt="" />
        </button>
      </div>
    </aside>
